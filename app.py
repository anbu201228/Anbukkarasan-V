...
@login_required
@role_required('approver1', 'approver2', 'approver3', 'approver4', 'admin')
...
def view_moc(moc_id):
    ...
    hours_pending_step1 = 1 # ✅ ALWAYS define first
    if moc.approver1_date:
        now = datetime.now(timezone.utc)
        approver_date = moc.approver1_date

        if approver_date.tzinfo is None:
            approver_date = approver_date.replace(tzinfo=timezone.utc)

        hours_pending_step1 = round(
            (now - approver_date).total_seconds() / 3600, 1
        )
    ...
    return render_template(
        ...
        hours_pending_step1=hours_pending_step1
    )
...
def admin_panel():
    ...
    if request.method == 'POST':
        ...
        if User.query.filter((User.username == username) | (User.employee_id == employee_id)).first():
            ...
            try:
                ...
                db.session.commit()
                flash(f'Successfully added {name} as {role}!', 'success')
            except Exception as e:
                db.session.rollback()
    ...
    # Get all users with approver or admin roles
    approvers = User.query.filter(User.role.in_(['approver1', 'approver2', 'approver3', 'approver4', 'approver5', 'approver6', 'admin'])).all()
...
MOC System
    ...
    return send_email(approver.email, subject, body, html_body)
...
def notify_submitter(moc_id, status):
    ...
    return send_email(submitter_email, subject, body)

def check_escalations():
    """Check for pending approvals > 3 hours and escalate"""
    three_hours_ago = datetime.now(timezone.utc) - timedelta(hours=1)
